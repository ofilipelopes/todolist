...
import br.com.filipe.todolist.utils.Utils;
...
public class TaskController {
    ...
    public ResponseEntity create(@RequestBody TaskModel taskModel, HttpServletRequest request) {
        ...
        if (currentDate.isAfter(taskModel.getStartAt())) {
            return ResponseEntity.status(400).body("A data de início deve ser após a data atual");
        }

        if (taskModel.getStartAt().isAfter(taskModel.getEndAt())) {
            return ResponseEntity.status(400).body("A data de término deve ser após a data de início");
        }
